...
## 2026-09-17 — Deleting an `auth.users` row bricks that install: the orphaned session is cached locally and never revalidated

**WHAT:** Wiping the Tier 2 test data (truncating `public.users`/`devices`/`invitations`/etc. plus `delete from auth.users`) left both test devices unable to register, with no in-app recovery path. `ensureSession()` (`artifacts/mobile/services/SessionService.ts`) returned the locally-cached session without ever validating it, so the app kept presenting a JWT whose `auth.users` row no longer existed: `GET /auth/v1/user` → 403, and `POST /auth/v1/token?grant_type=refresh_token` → 400 because the refresh token was orphaned too. `autoRefreshToken` could not recover it. Fixed in `2ea569d` — `ensureSession()` now revalidates a cached session via `getUser()` and signs out + re-mints anonymously when the user is gone. A network-shaped error deliberately resolves to "still valid" so an offline device never discards a good account.

**WHY non-obvious:** Three separate traps here. (1) **"Clear cache" is not "Clear storage"** on Android — the user cleared cache first, which leaves AsyncStorage (and therefore the dead JWT) completely intact; only "Clear storage" fixed it. (2) The edge logs showed `self_register` returning **400 before the wipe** (22:34, 00:56, 01:05) — a *pre-existing* failure from `selfRegister.sql`'s collision guard (raises `P0001` when a `phone_hash` belongs to a different user id), which the wipe actually *cleared*. It would have been easy to conflate that 400 with the post-wipe 403 and "fix" the wrong thing. (3) The two failures are only distinguishable by timestamp and user-agent: `Deno/...` is the Edge Function calling PostgREST, `okhttp/...` is the app itself. The decisive evidence was the 400 on the refresh grant from `okhttp` — proof the app was actively trying and failing to recover on its own, not merely holding a stale token.

**WHERE:** `artifacts/mobile/services/SessionService.ts` (`ensureSession`, new `cachedUserStillExists`), tests in `SessionService.test.ts` (4 new, sabotage-checked: reverting the guard fails only the regression test). Branch `fix/ensure-session-revalidate`, not yet merged or pushed. **Unproven on hardware** — verified in Jest only; the real path (orphaned token → revalidate → fresh session → successful `self_register`) has not been exercised on a device, and no `device-tests/` entry was added. To confirm it for real: wipe the backend again and watch a device recover *without* clearing storage. Note the general rule for any future test-data reset — clearing server rows alone strands every already-authenticated client, so plan on clearing app storage per device unless/until this fix is verified live.
