...
## 2026-09-23 — B9 reset/migrate "didn't work" on device because nothing was deployed; every test layer mocked its neighbour

**WHAT:** On a real device, the new collision screens appeared, but confirming "migrate" still ended in "That number is already registered". Checked against the live `remindme-tier2` project: `self-register` was still at **version 1**, with no `action` field and always calling `self_register()`. `reset_phone_number` and `migrate_phone_number` did not exist in `pg_proc`. The old function ignored `action: "migrate"`, ran plain registration, hit the same collision and returned `number_taken`. The branch was committed, pushed and green; nothing had been deployed. The exit report called the remaining gap "unverified on hardware", which hid that the backend had never received the change.

**WHY it slipped through:** (1) Each test layer stubs the next one. Jest mocks `selfRegister`, the Deno test fakes the Supabase client, and PGlite applies the repo's own SQL. All three passed, and none of them looks at what production actually runs. (2) CLAUDE.md already required `push:sql` plus an Edge Function redeploy after such changes, but nothing made "committed" and "deployed" separate states in a report. (3) Adding a field to an existing endpoint fails silently against an old server: the field is dropped and the old code path's error comes back, so it looks like a logic bug rather than a stale deploy.
