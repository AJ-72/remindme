...
build. Ordered cheapest-first; B16 depends on the outcomes of B10, B12 and
B15 below.
...
| B13 | — | Give the "we don't verify this number yet" caveat its own visual weight | S | `OPEN` | `app/register-number.tsx:290–294`, non-first-run message body. The caveat ("We don't verify it with a code yet — just don't use someone else's number") is present and accurate, but reads as a continuation of the sentence before it, so a skimming user can miss the one sentence that carries real consequence (a wrong number can already be claimed by someone else — see `number_taken`). Bold it or put it on its own line. Copy itself needs no rewrite. |
| B15 | — | Confirm whether the exact-alarm settings redirect fires in practice, then act | S (investigation), then S–M | `OPEN` | `app/_layout.tsx:136–139` redirects to system Settings only when `checkExactAlarmPermission()` (`services/ReminderService.ts:777–809`) returns exactly `false` — it returns `null` on iOS, on Android below API 31, or if the native check is unavailable. The app's own Settings explainer (settings.tsx:394–400) describes `USE_EXACT_ALARM` as "normal, auto-granted, non-revocable" on a modern targetSdk build, which suggests this branch may rarely or never fire on a shipped build. First step: verify on a real Android 12+ device whether it fires at all pre-grant. If it does, drop the forced first-launch redirect (no return message today) and let the existing, dismissible `ExactAlarmBanner` carry this instead — it already reappears correctly via its `AppState` listener. |
...
| B14 | — | Turn the About screen into a feature-discovery surface | S–M | `OPEN` | `app/(tabs)/about.tsx` is currently a name, a tagline, and a version number — no mention of voice dictation, Malayalam support, the alarm-vs-silent distinction, or "remind someone else". (Settings already names most of these in row sub-labels, so this is a secondary gap, not the only place they're explained.) Add a short "What you can do" list, 4–5 lines, one capability per line in plain language (e.g. "Speak a reminder in English or Malayalam," "Have someone remind you back"). |
| B16 | — | Defer name/phone asks until after the user's first reminder | M | `OPEN` | Depends on B10, B12 and B15 landing first (each removes or reframes one link in the same chain). Today a fresh install can see, in order: a bare notification-permission dialog, an optional exact-alarm redirect, a name-request sheet, and a phone-number modal — all before the user has created a single reminder or seen what the app does (`app/_layout.tsx:129–146`, `components/NameOnboarding.tsx`). Both the name sheet and the phone screen already have durable second chances (header avatar, Settings row), so neither needs to be first-run at all. Target: request notification permission at the moment it's earned (right after reminder #1 is saved, framed as "so this actually reminds you"), and drop name/phone from the first-launch path entirely. Needs its own short design pass before implementation — this is a sequencing change across `_layout.tsx`, not a copy fix. |
