...
| B15 | — | Multiple pending invitations: grouped notification + a "Pending reminders" list screen | M | `OPEN` | Today, N pending invitations means N separate full-detail pushes, each requiring its own tap → `invitation-preview.tsx` → Accept/Decline round trip — no way to see them together. **Multiple senders is not a distinct case today, and won't be under this design either**: `claim_invitations()` (the DB RPC behind `claim-invitations`) already returns *all* pending invitations for the caller in one flat list regardless of how many different senders they're from — there is no per-sender grouping at the API layer, only a count. The actual gap is entirely client-side: `checkForInvitations()` (`InvitationService.ts`) auto-navigates only when the claimed list is length exactly 1; 2+ (whether 2 from one sender or 5 from five senders — indistinguishable to this check) get claimed (marked delivered) but the app currently shows nothing for them at all. **Design (approved 2026-09-11):** the first push for a newly-idle recipient stays as today ("{name} sent you a reminder"); a second-or-later *concurrent* pending invitation instead renders as a grouped/updated notification ("{name} + N more reminders waiting") rather than stacking another full push. Tapping any invitation notification opens a new list screen (not `invitation-preview.tsx` directly) showing every currently-pending invitation at once — mixed senders included, one row per invitation with the sender's name on that row (reusing/batching `get_sender_display_name`-style resolution) and inline Accept/Decline — no per-item drill-in required, and no separate design needed for the same-sender vs. different-senders case since the list treats every row identically regardless of origin. Needs: (a) tracking "how many invitations are currently unresolved for me" — likely a `claim-invitations` response shape change or a new lightweight count RPC; (b) Android notification grouping (`expo-notifications`' `channelId`/tag-based grouping, or collapsing via re-posting the same identifier) for the push side; (c) the new list screen itself, reusing `invitation-preview.tsx`'s Accept/Decline logic but list-rendered, resolving each row's sender name (batch the RPC rather than one call per row); (d) deciding what happens to `useInvitationCheck.ts`'s existing "navigate only if exactly one claimed" rule — this item is what finally builds the "still-unbuilt list screen" that rule has deferred to since Tier 2 shipped. Needs a short design pass on the count/grouping mechanism before implementation (the UI shape itself is approved; the plumbing to know "how many are pending" without an extra round trip per push is not yet decided). |
