...
| B22 | — | Secret-answer factor for the phone-migration rebind window | M | `OPEN` | Raised 2026-09-19 alongside a bug fix (`self_register()` could not reclaim a number from an account inactive under 45 days, since the account had no verified way to prove it was the same person — see `lib/db/src/functions/selfRegister.sql`'s short-term fix, which only lets a STALE (45+ day) account be reclaimed, and only as a fresh account with no blocks/links carried over). Idea: at registration, the user sets a secret answer (hashed server-side, never sent in plaintext). On a later rebind, requiring the correct answer ALONGSIDE OTP (T2.3) would let a rebind recover the OLD account, blocks and links included, even on a NEWLY-recycled number, because a stranger holding a carrier-recycled number would not know the answer either. Needs: a `users` column for the hashed answer, a new SECURITY DEFINER function to set/verify it (never a plain compare - same rules as every other function in this file: no argument the caller could lie about, search_path pinned, schema-qualified), and a design decision on what happens if the answer is forgotten (a secret question is a known-weak, unrecoverable factor - see design discussion 2026-09-19). Depends on T2.3 (OTP) landing first; the answer check should not stand alone as the only factor. |
