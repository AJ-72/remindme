...
| B14 | — | B11's fix never reached the live `send-invitation` Edge Function | S | `OPEN` | Found 2026-09-11: `handleSendInvitation`'s code (repo, commit `5ce5677`) already builds `"{name} sent you a reminder"` from `senderId`'s `display_name` and tags `data.type: "invitation"`, but `mcp__Supabase__get_edge_function` on the live `remindme-tier2` project shows the deployed function is still the pre-B11 version — generic `"New reminder"` title, no `senderId` param, no `data.type` tag at all. This is a deploy gap, not new design work: redeploy `send-invitation` (`scripts/deploy-edge-functions.ps1 -Function send-invitation` with `SUPABASE_ACCESS_TOKEN`, or `mcp__Supabase__deploy_edge_function` as the documented fallback — see CLAUDE.md Gotchas) and re-verify via `get_edge_function` that the live source matches the repo. Also worth an audit of the other 4 functions (`lookup`, `claim-invitations`, `respond-invitation`, `expire-invitations-cron`) for the same drift, since nothing currently catches an Edge Function silently going stale after a merge. |
| B15 | — | Multiple pending invitations: grouped notification + a "Pending reminders" list screen | M | `OPEN` | Today, N pending invitations from different senders means N separate full-detail pushes, each requiring its own tap → `invitation-preview.tsx` → Accept/Decline round trip — no way to see them together. **Design (approved 2026-09-11):** the first push for a newly-idle recipient stays as today ("{name} sent you a reminder"); a second-or-later *concurrent* pending invitation instead renders as a grouped/updated notification ("{name} + N more reminders waiting") rather than stacking another full push. Tapping any invitation notification opens a new list screen (not `invitation-preview.tsx` directly) showing every currently-pending invitation at once, each row with inline Accept/Decline — no per-item drill-in required. Needs: (a) tracking "how many invitations are currently unresolved for me" — likely a `claim-invitations` response shape change or a new lightweight count RPC; (b) Android notification grouping (`expo-notifications`' `channelId`/tag-based grouping, or collapsing via re-posting the same identifier) for the push side; (c) the new list screen itself, reusing `invitation-preview.tsx`'s Accept/Decline logic but list-rendered; (d) deciding what happens to `useInvitationCheck.ts`'s existing "navigate only if exactly one claimed" rule — this item is what finally builds the "still-unbuilt list screen" that rule has deferred to since Tier 2 shipped. Needs a short design pass on the count/grouping mechanism before implementation (the UI shape itself is approved; the plumbing to know "how many are pending" without an extra round trip per push is not yet decided). |
