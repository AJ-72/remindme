...
## Tier 0b — onboarding & first-launch UX (independent, no prerequisites)

From the 2026-09-11 UX review of first launch → registration → Settings →
home empty state (Artifact: "Onboarding Under Review", fact-checked against
source same day). All items are UI/copy-level and need no backend or native
build. Ordered cheapest-first; B16 depends on B12 and B15 landing first (see
B16's own note — its scope was corrected after B10 turned out not to
reproduce against current source).

| # | Legacy # | Item | Effort | Status | Notes |
| --- | --- | --- | --- | --- | --- |
| B10 | — | Delete the first-run auto-push to register-number.tsx | S | `UNVERIFIED — likely stale finding` | The review's lead "critical" finding claims `app/_layout.tsx:151–161` auto-pushes `/register-number` with `firstRun: "1"` on every fresh install, contradicting the screen's own header comment (lines 26–38) that says it should be "reachable from Settings… rather than auto-prompted on first launch." **Checked against the current source 2026-09-11: no such push exists.** `_layout.tsx` is 194 lines total, has no `firstRun` reference anywhere, and `git log -S"firstRun"` on this file returns no commits. The only navigation to `/register-number` is the Settings row (`app/(tabs)/settings.tsx:284`), which matches the comment. Left open rather than deleted per instruction, in case this is being checked against a different branch/build than `main` — **re-confirm against the actual branch the review was run on before treating this as real work.** If it doesn't reproduce, close as not-a-bug. |
| B11 | — | Label the "remind someone else" icon in QuickAddInput | S | `OPEN` — implementation-ready | Verified 2026-09-11: `components/QuickAddInput.tsx:860` (`quick-add-mic`) and `:880` (`quick-add-recipient`) render `Feather` icons in the same row; the recipient button swaps `user-plus`/`user-check` (`:883`) by whether a recipient is already attached. No visible text label today, only an `accessibilityLabel`. **Plan:** persist a `hasSeenRecipientHint` flag via `ReminderService` (same pattern as `hasSeenNamePrompt`/`markNamePromptSeen` in `NameOnboarding.tsx`). On first mount of `QuickAddInput` where the flag is unset and `!recipient`, show a small one-time callout anchored to the `quick-add-recipient` button (a `Pressable` + `Text` bubble is enough — no new dependency needed) reading "Tap to remind someone else too." Dismiss on outside tap or after the user interacts with the button; call `markRecipientHintSeen()` either way, mirroring `NameOnboarding`'s close(). Do **not** add example phrases like "Remind Priya tomorrow at 9am" to any copy — `parseNaturalLanguage.ts` does not parse recipients from free text; they're attached via the contact picker only. Add a render test asserting the hint shows once and not on a second mount (mock the AsyncStorage flag), following the pattern in `__tests__/screens/register-number.test.tsx`. |
| B13 | — | Give the "we don't verify this number yet" caveat its own visual weight | S | `OPEN` — implementation-ready | Verified 2026-09-11: the caveat lives in the single `input`-phase message at `app/register-number.tsx:216–220` (one `Text` block, two sentences) — there is no separate "non-first-run" version, since (per B10) there's no first-run/returning-user split in the current screen at all; the review's framing of this as "the non-first-run version" doesn't quite match, but the underlying issue is real: "We don't verify it with a code yet — just don't use someone else's number" reads as a continuation of the sentence before it. A wrong number can already be claimed (`copyForError`, line 73: "That number is already registered to a different account"). **Plan:** split the single `Text` at line 216 into two: keep the first sentence in the existing `styles.message`, and put the caveat in a new sibling `Text` styled with `colors.warningSurfaceForeground` (the token `constants/colors.ts:66` already reserves for "advisory, non-error notices" — the exact category this is) and `Inter_500Medium` instead of `Inter_400Regular`, on its own line below. No copy rewrite needed. Add/extend a render test in `__tests__/screens/register-number.test.tsx` asserting the caveat renders as a distinct text node. |
| B15 | — | Confirm whether the exact-alarm settings redirect fires in practice, then act | S (investigation), then S | `OPEN` — implementation-ready | Verified 2026-09-11, real line refs: `checkExactAlarmPermission()` (`services/ReminderService.ts:713–725`) reads `Notifications.getPermissionsAsync().android.alarm`, which reflects `AlarmManager.canScheduleExactAlarms()`; the redirect fires from `app/_layout.tsx` (inside the first-launch effect, ~line 130) only when this returns exactly `false`. **Strong evidence it's dead on a shipped build**: `settings.tsx:394–400`'s own code comment records an on-device check (2026-08-29) that this app holds `USE_EXACT_ALARM` — "a normal, auto-granted, non-revocable permission that supersedes `SCHEDULE_EXACT_ALARM` on targetSdk 34+" — and `app.json` declares both `android.permission.SCHEDULE_EXACT_ALARM` and `android.permission.USE_EXACT_ALARM`. If `USE_EXACT_ALARM` is auto-granted at install, `alarm` should already read `true` the first time this effect runs, so the `false` branch (and therefore the redirect) should not fire on a targetSdk-34 build at all. **Plan:** (1) confirms this on a real Android 12+ device — log or breakpoint `checkExactAlarmPermission()`'s return value on a fresh install, before assuming; (2) if confirmed dead, delete the redirect block in `_layout.tsx`'s first-launch effect (the `if (exactAlarmGranted === false) openExactAlarmSettings();` lines) and the now-unused `openExactAlarmSettings` import there — leave the function itself in `ReminderService.ts`, since `ExactAlarmBanner.tsx:40` still calls it from its "Fix in Settings" link, which is the correct, already-dismissible, already-`AppState`-aware home for this control. No test changes needed beyond removing any test asserting the first-launch redirect call, if one exists. |
| B12 | — | Add a one-line pre-ask screen before the notification permission dialog | S–M | `OPEN` — implementation-ready | Verified 2026-09-11: `requestNotificationPermissions()` (`services/ReminderService.ts:535–553`) calls `Notifications.requestPermissionsAsync()` (line 542) with no app screen before it anywhere in the chain; called from `app/_layout.tsx:130` inside the first-launch effect (lines ~122–140), and again from `scheduleNotification()` (`ReminderService.ts:576`) on every save — the in-flight guard (`permissionRequestInFlight`, lines 528–553) already makes it safe to call from both places without a double prompt. **Plan:** add a new `components/NotificationPrimerSheet.tsx`, modeled directly on `NameSheet.tsx` (same `Modal`/overlay/sheet styling, `useColors()`, `skippable` pattern) with a single message: "Reminders needs to notify you — that's the whole app." and one button ("Continue") that calls `requestNotificationPermissions()` then closes. In `_layout.tsx`, gate the existing first-launch effect so the primer renders before the first call to `requestNotificationPermissions()` — same `readyForNamePrompt`-style boolean, sequenced ahead of `NameOnboarding` so only one sheet is ever stacked on the OS dialog (reuse the existing comment's reasoning at `NameOnboarding.tsx:12–14` about a sheet rendering behind a system dialog). No new persisted flag needed — `hasCompletedPermissionOnboarding()`/`markPermissionOnboardingComplete()` already gate this whole effect to once per install. Add a render test asserting the primer shows before the permission call fires (mock `Notifications.requestPermissionsAsync`) and a snapshot/copy test, following `__tests__/components/` conventions. |
| B14 | — | Turn the About screen into a feature-discovery surface | S–M | `OPEN` — implementation-ready | Verified 2026-09-11: `app/(tabs)/about.tsx` is 78 lines — a header, an icon, `CuriousMind Labs`, the tagline `Be Curious`, and a version number (lines 62–76), nothing else. Covered today by one test (`__tests__/screens/about.test.tsx`) asserting exactly those three strings. **Plan:** add a `styles.capabilityList`/`styles.capabilityRow` block below the existing `version` Text, each row a `Feather` icon + one line of copy, reusing the icon-plus-label visual weight already used elsewhere (e.g. Settings rows) rather than inventing a new pattern. Five rows, one capability each, in the user's language: "Speak a reminder in English or Malayalam" (mic icon), "Malayalam text works throughout the app" (type icon), "Have someone remind you back" (user-plus icon — same glyph as B11's QuickAdd button, reinforcing it), "Choose alarm-style or silent notifications" (bell icon), "Back up and restore your reminders" (Settings already has this row — download icon). Keep each line under one sentence; no paragraph copy. Extend `about.test.tsx` with assertions for the five new strings. No dependency on B10–B13/B15/B16 — purely additive to a screen nothing else touches. |
| B16 | — | Defer the name ask until after the user's first reminder | M | `OPEN` — implementation-ready, scope corrected | Depends on B12 and B15 landing first (each removes or reframes a link in the same chain); does **not** depend on B10 — see correction below. **Scope correction from the review:** re-checking the actual chain (`app/_layout.tsx`'s first-launch effect, ~lines 122–146) against B10's finding, only **two** real first-launch interruptions exist today, not the three–four the review describes: the bare notification-permission dialog (B12's target) and the `NameOnboarding` sheet (`components/NameOnboarding.tsx`) — there is no first-launch phone-number ask (B10 found no such push exists). So this item is now scoped to deferring the **name sheet only**, not name-and-phone. **Plan:** the name sheet already has a durable second chance — `app/(tabs)/index.tsx`'s header avatar (lines 256–322, 428–435) opens the same `NameSheet` any time, tap-to-add if no name is set — so nothing is lost by dropping it from first launch. Remove the `<NameOnboarding enabled={readyForNamePrompt} />` render from `_layout.tsx` (and, once unused, `readyForNamePrompt`/`hasSeenNamePrompt`/`markNamePromptSeen`'s first-launch call sites — keep the functions themselves, `index.tsx`'s own edit-name flow doesn't need the "seen" bookkeeping so check nothing else reads it first). Notification permission already re-requests at save time via `scheduleNotification()` → `requestNotificationPermissions()` (`ReminderService.ts:576`), so once B12 lands, the first-launch effect can shrink to just showing the primer sheet + requesting permission (or, if B15 confirms the exact-alarm branch is dead, to just the primer). Needs a quick before/after walk of `_layout.tsx`'s effect once B12 and B15 are actually merged, since this item edits the same function both touch — sequence it after both, not in parallel. |
