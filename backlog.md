...
| M9 | — | Smart re-nudge (re-alert ladder) | L | `OPEN` | Prerequisite (real `snoozeCount` data) is now met; ready to spec. See [roadmap.md#m9](docs/roadmap.md#m9-smart-re-nudge). **Design tension flagged 2026-09-22**: the spec caps at 3 escalations and deliberately backs off past `snoozeCount >= 3` (the "dread override"). Market research (nag-app reviews, ADHD forum threads) shows a distinct, vocal user segment wants the opposite — an alert that keeps firing every N minutes until explicitly dismissed, with no ceiling (this is Nag Reminder's/Alarm Clock Xtreme's entire pitch). Not a contradiction to fix, but worth deciding explicitly: either document M9's gentler default as an intentional differentiator from that segment, or add an opt-in "persistent" tier above "Persistent" with no hard stop. Needs a product decision, not more spec work. |
| M3 | — | Location-based reminders | L | `OPEN` | See [roadmap.md#m3](docs/roadmap.md#m3-location-based-reminders). |
| B26 | — | Delivery-reliability self-check ("will this reach me?") | M | `OPEN` | Raised 2026-09-22 from market research (see chat log / this entry's context): the single most common complaint across Play Store reviews, Reddit, and forum threads about reminder apps generally — not this app specifically — is silent non-delivery: exact-alarm permission denied by default since Android 14, OEM battery-optimization killing the background process, or notification permission denied once at onboarding, each of which discards reminders with no visible symptom until the user notices something didn't fire. `ExactAlarmBanner.tsx` already exists and is a partial, passive mitigation (surfaces one of these three conditions). Proposed scope: a single Settings screen or card that actively checks and reports on all three conditions (exact-alarm permission, battery-optimization exemption, notification permission/channel importance) with a real test-fire button that schedules and confirms a notification arrives within seconds, not just a static permission-granted check. Positions reliability as a visible product surface instead of an apology banner. No design/spec work started. |
| B27 | — | Escalate to a second person on missed acknowledgment | L (depends on M4 Tier 2 + M9) | `OPEN` | Raised 2026-09-22 from market research: caregiver-facing medication apps (Medisafe's "Medfriend", TendTo, SteadiDay) build their retention around a trusted second person being notified when the primary recipient doesn't acknowledge a reminder — Medisafe cut its free tier to 2 medications in Jan 2026, leaving an actively frustrated user base. This app has the two prerequisite pieces already shipped (M4 Tier 2 send-to-another-person, live since 2026-09-11; M9's re-alert ladder concept, spec'd) but nothing wires a missed *recipient*-side reminder back to notifying the original *sender*. Distinct from [B16](—) (auto-accept notifications, which is about the sender's send succeeding, not the recipient's follow-through) and from M9 (which re-nudges the same person, not a different one). Needs a design pass: what counts as "missed" (no `openedAt`? not marked complete after N escalations?), what the sender sees, and whether this is opt-in per-reminder or a relationship-level setting between two paired users. Not scoped further; needs a decision before it's spec-ready. |
...
| B24 | — | Android home-screen widgets: quick add + upcoming list | M | `OPEN` | Two widget surfaces on the Android home screen: (1) a **quick-add** widget with a text field and a mic button, and (2) an **upcoming list** widget showing the next few reminders with a quick-add button. Not started. Needs a native config plugin plus an Android App Widget provider (`AppWidgetProvider` + `RemoteViews`, or Glance) — React Native cannot render a widget, so the UI is native Android code, and the widget process cannot read AsyncStorage directly. Design decisions still open: how the widget reads reminder data (a shared-preferences/JSON mirror written by `ReminderService` on every write, vs. a `SharedPreferences` bridge module), whether quick-add saves directly or deep-links into `add-reminder.tsx` with the text prefilled, how dictation works from the widget (the mic likely deep-links into the app rather than running `expo-speech-recognition` in the widget process), Malayalam font rendering in `RemoteViews`, and widget refresh on data change. Blocked on a native build like the rest of this tier; also needs device tests added to `device-tests/` since a widget is invisible to Jest. |
| M10 | — | Voice reminders (the setter's own recorded voice as the notification sound) | L | `OPEN` (brainstormed 2026-09-25, needs spec) | Step 1: voice on a local reminder; step 2: send the clip with an M4 Tier 2 reminder. Real recordings only, no AI voice. **Starts with an Android spike** — `expo-notifications` only plays sounds bundled at build time, so a runtime clip likely needs a native per-clip channel; if the spike fails, stop and decide before building further. See [roadmap.md#m10](docs/roadmap.md#m10-voice-reminders). |
...
| M11 | — | Habits and care nudges (habits for yourself or a family member) | L | `OPEN` (brainstormed 2026-09-25, needs spec) | After M10. A habit is one type with a check-off and a soft consistency measure ("5 of the last 7 days", not a hard streak); a care nudge is a habit set for someone else over M4 Tier 2, with an optional M10 voice clip. **Needs a decision on the "adherence is derived, not logged" rule first** — a per-day consistency view needs per-occurrence history the record does not keep today. See [roadmap.md#m11](docs/roadmap.md#m11-habits-and-care-nudges). |
