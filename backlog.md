...
| B28 | — | Drop `LEGACY_SNOOZE_ACTION_ID` (`"SNOOZE_10"`) | S | `BLOCKED` | B5 renamed the quick-Snooze action id and kept the old value in the handler for notifications already posted before the upgrade (`services/notificationResponseHandler.ts`). Remove the constant, its handler branch and its test once **one release has shipped** carrying B5, since no pre-B5 notification can still be sitting in a tray by then. Blocked on that release. |
