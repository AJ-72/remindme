 ...
 *
 * push handles tables, indexes and constraints. It does NOT handle functions
 * or grants — and both carry security properties here. (It is ALSO not
 * proven to reliably carry policy expressions — see verifyRls.ts and
 * system_learnings.md's 2026-09-08 entry — which is why `deploy` chains
 * `verify:rls` after this script, not why this script does anything
 * differently; policies remain declared in the Drizzle schema, not here.)
 *
