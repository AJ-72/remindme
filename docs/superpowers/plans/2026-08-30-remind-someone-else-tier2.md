...
| T1.9 | `drizzle-kit push` wired and documented | **DONE 2026-09-07.** `drizzle.config.ts` sets `entities.roles.provider = "supabase"`, without which drizzle-kit proposes managing `authenticated`/`anon` — roles it did not create — up to and including dropping them. `push` then `push:sql` both ran clean against `remindme-tier2`. Two real bugs found and fixed in the process — see "T1.9 — two bugs found deploying against a real Supabase project" below |
