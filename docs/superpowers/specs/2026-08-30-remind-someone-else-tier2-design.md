...
- **Consume the token on *claim by the app*, never on link resolution.** WhatsApp
  ...
  **Implemented 2026-09-07 as `bind_via_invite_token(token)`.** `invitations.bind_token`
  ...
  is never read aloud — it only ever travels inside a URL. Single-use is
  tracked on `bound_by`/`bound_at` on the invitation itself, set by one atomic
  `UPDATE`, not inferred from `users.phone_hash` being unique — the first
  version did that, and an independent review found it meant deleting the
  bound account (a Play Store requirement) freed the token for a stranger to
  claim. The function does not itself defend against the `GET`-burns-the-token
  trap above; that property belongs to whichever Edge Function endpoint
  WhatsApp's crawler actually fetches, not yet built.
...
- **The claim query cannot be protected by ordinary RLS.** "Give me every invitation matching my phone hash" runs against rows that are not yet owned by anyone, so no row-ownership policy covers it. It needs a `SECURITY DEFINER` function with its own explicit checks — and it is the most attack-exposed endpoint in the design, because it is the one #1 abused. It deserves more scrutiny than anything else here.
  ...
  **Implemented 2026-09-07 as `claim_invitations()`.** The resolution to both problems above is the same one thing: **the function takes no argument.** It reads the caller's own `users.phone_hash` rather than accepting one, so there is nothing to lie about and no input to probe with — which is what makes it neither a squatting vector nor an enumeration oracle. A `claim_invitations(hash)` signature would have been both. It also refuses rows whose content retention has already lapsed, which closes most of a gap found while building it: an **unclaimed** invitation has a null `recipient_id` and therefore does *not* cascade when the account for that number is deleted, so a recycled number's new owner could otherwise inherit their predecessor's mail. The complete fix — purging unclaimed rows for a hash when a *fresh* account binds it — belongs in binding, and is tracked as T1.10.
