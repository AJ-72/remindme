...
**Write serialization**: `services/ReminderService.ts` has a private `withWriteLock()` queue guarding `rescheduleAllFutureReminders()`, `markNotifiedById()`, and `markOpenedById()` — added after a real race surfaced building the instrumentation above. All three do an independent "load the full array, mutate one reminder, save the full array" cycle, and nothing serialized them: a killed app cold-started straight into `reminder-detail` via a notification tap runs the provider's mount-time reschedule sweep and the screen's own open-stamp at the same moment, and whichever finished saving *last* used to win, silently discarding the other's write. **Not yet applied to every writer in the file** (`addReminder`, `editReminder`, `deleteReminder(s)`, `toggleComplete`, `snoozeReminder`, `updateSnoozeById` all share the same class of risk in theory) — only the three above are known to actually run concurrently with each other, which is what made the bug observable. Widening the queue to cover every writer is unstarted, not assumed done.
