...
**One real bug-shaped gap the pilot found and fixed**: `medianSlipMinutes`' even-length branch, `(sorted[mid - 1] + sorted[mid]) / 2`, could be mutated to `* 2` with every existing test still passing — the only pre-existing test for this field had a single completion, never reaching the even-length averaging branch at all. Fixed in `utils/adherenceStats.test.ts` with a two-completion case that pins the exact average; sabotage-checked by hand (reverting to `* 2` now fails it), matching the same sabotage-check discipline this file already asks for on RLS tests.
