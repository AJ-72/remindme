...
**Write serialization**: `services/ReminderService.ts` has a private `withWriteLock()` queue guarding `rescheduleAllFutureReminders()`, `markNotifiedById()`, `markOpenedById()`, and `markDoneById()` — added after real races surfaced building the instrumentation above and, for `markDoneById()`, the notification-tray "Mark Done" path (see `system_learnings.md`, 2026-09-23 entry). All four do an independent "load the full array, mutate one reminder, save the full array" cycle, and nothing serialized them: a killed app cold-started straight into `reminder-detail` via a notification tap runs the provider's mount-time reschedule sweep and the screen's own open-stamp at the same moment (or, for Mark Done, the same sweep racing the tray action after a body tap already launched the app — D15's own scenario), and whichever finished saving *last* used to win, silently discarding the other's write. **Not yet applied to every writer in the file** (`addReminder`, `editReminder`, `deleteReminder(s)`, `toggleComplete`, `snoozeReminder`, `updateSnoozeById` all share the same class of risk in theory) — only the four above are known to actually run concurrently with each other, which is what made the bugs observable. Widening the queue to cover every writer is unstarted, not assumed done. Note the lock only serializes writes *within one JS runtime* — it does not, and cannot, close the separate cross-process race between the headless `TaskManager` task and the foreground listener (see `system_learnings.md`'s 2026-08-23 "Whole-array read-modify-write races" entry); that gap is mitigated, not eliminated, by `RemindersContext`'s AppState-`active` reload.
