...
## Mutation testing and property-based testing (mobile)

A pilot ran 2026-09-23 on the three most date/logic-heavy `utils/` files (`quietHours.ts`, `recurrence.ts`, `adherenceStats.ts`), comparing mutation testing (Stryker) and property-based testing (fast-check) against Lean and TLA+ for this codebase. Full analysis and before/after mutation scores: `docs/knowledge/` does not yet carry this — see the session's published report (ask the user for the link, or re-run the pilot per this section) for the full comparison table and reasoning. Summary of what was decided and why:

- **Use both, together, going forward for new date/text logic in `utils/`.** Mutation testing finds *where* a file's tests are weak (a changed operator/constant that no test catches); fast-check property tests make specific rules *strong* against many random inputs at once, rather than one hand-picked example. They are complementary, not redundant — see the pilot's real result below.
- **Do not adopt Lean.** A Lean proof would apply only to a from-scratch Lean re-implementation of the algorithm, never to the TypeScript that actually runs — it cannot catch a bug where the two diverge, and this codebase's real risk (JS `Date`, local time, `TZ`) has no practical Lean model.
- **TLA+ is not adopted yet.** It has real fit for one specific gap: `ReminderService.ts`'s `withWriteLock()` covers only 3 of roughly 15 `saveReminders()` call sites (see "Write serialization" above) — a model of the unlocked writers could show a lost-update trace before the lock is widened. Revisit when that widening work starts; don't reach for it before then.

**Setup** (`artifacts/mobile`): `@stryker-mutator/core` + `@stryker-mutator/jest-runner` + `fast-check` as devDependencies; `stryker.conf.json` (packageManager `pnpm`, `plugins: ["@stryker-mutator/jest-runner"]` — plugin auto-discovery did not find the jest-runner plugin under pnpm's `.pnpm` store layout without this being explicit; `coverageAnalysis: "perTest"` so each mutant only reruns its related tests instead of the whole suite). Run via `pnpm --filter @workspace/mobile run test:mutation` (`stryker run`). **Not added to any CI gate** — a 3-file run takes 60-90 minutes (Stryker's coverage-analysis dry run executes the full Jest suite once up front, then hundreds of per-mutant reruns). `.stryker-tmp/` and `reports/` are gitignored.

**Baseline pilot result** (before → after adding fast-check properties, 3 files, 823 mutants): `quietHours.ts` 98.15% → 98.15% (already well covered), `recurrence.ts` 78.14% → 78.32%, `adherenceStats.ts` 72.30% → 74.18%. The gain is small in aggregate because the new properties targeted specific functions (`computeNthOccurrence`, `isValidRecurrenceRule`, `quietHoursEndAfter`, `completionRate`'s `MIN_SCORED_FOR_RATE` floor) inside large files whose biggest remaining gaps are elsewhere: `parseRecurrencePhrase()`'s regex table (every test phrase happens to use a single space, so `\s+` vs `\s` mutants survive en masse — low severity, one two-space test phrase would kill the whole class at once) and `adherenceStats.ts`'s hour/weekday bucket-tallying arithmetic.

**Two real bug-shaped gaps the pilot found and fixed, both in `adherenceStats.ts`, both sabotage-checked by hand**: `medianSlipMinutes`' even-length branch, `(sorted[mid - 1] + sorted[mid]) / 2`, could be mutated to `* 2` with every existing test still passing — the only pre-existing test for this field had a single completion, never reaching the even-length averaging branch at all. Fixed in `utils/adherenceStats.test.ts` with a two-completion case that pins the exact average. Second: the `bestHour`/`worstHour` tie-guard (`best.hour !== worst.hour`), the `enoughData` threshold (`scored >= MIN_SCORED_FOR_HOUR_ADVICE && eligible.length >= 2`), the per-bucket eligibility filter, the tie-break sort, the on-time boundary (`slipMs <= ON_TIME_WINDOW_MS`), and the `byHour`/`byWeekday` bucket-tallying arithmetic all had surviving mutants no test caught — worth fixing (unlike `recurrence.ts`'s regex-whitespace survivors, see below) because this is exactly the "stay silent absent evidence" principle in "Adherence is derived, not logged" above, feeding user-facing copy in `insights.tsx`. Six targeted tests added, each individually sabotage-checked (the six mutants above each now fail one specific test when reintroduced by hand).

**`recurrence.ts`'s 100 remaining survivors and `adherenceStats.ts`'s other ~48 were deliberately left alone** — before writing tests against them, they need an equivalent-mutant review pass this pilot did not do (most look like the regex-whitespace pattern above, or defensive-guard branches where the mutated behavior is numerically indistinguishable, e.g. adding `tallyCompleted ?? 0` when it's always 0 anyway). Don't assume every surviving mutant is a real gap; triage by category (as `CLAUDE.md`'s own pilot section shows) before spending test-writing effort on one.

**Extending the pilot**: widening `mutate` in `stryker.conf.json` to more of `utils/` (e.g. `parseNaturalLanguage.ts`, `malayalamDateParser.ts`) is a reasonable next step but was deliberately not done in this pilot — each additional large file adds tens of minutes per run, and equivalent-mutant review (a surviving mutant that cannot actually be killed because it doesn't change behavior) needs a human pass before trusting any score at face value.
...
**Adherence is derived, not logged.** `utils/adherenceStats.ts` computes every completion/timing statistic the app shows from the reminder records themselves — `createdAt`, `completedAt`, `snoozeCount`, `originalDatetime`. There is deliberately **no separate event log**: a second store of the same facts can disagree with the first, and the disagreement is unfixable after the fact. Accepted costs: events the record cannot witness (a notification fired and swiped away) are invisible, and deleting a reminder deletes its history. Two rules the module enforces and the tests pin: bucket by `originalDatetime ?? datetime`, never `datetime` alone (snoozing overwrites `datetime`, so it would let a task rewrite which hour it was set for); and return `null` rather than a number whose sample cannot support it — `MIN_SCORED_FOR_RATE`, `MIN_SCORED_FOR_HOUR_ADVICE`, `MIN_BUCKET_FOR_HOUR_ADVICE` are the floors, and `utils/adherenceCopy.ts` renders each `null` as an em dash plus a sentence saying what is still missing. `suggestBetterHour()` additionally stays silent unless it has evidence *against* the hour the user picked; absence of data is not an argument for moving someone's reminder. Consumers: `app/insights.tsx`, the save-time banner in `app/add-reminder.tsx`, and the postponement panel in `app/reminder-detail.tsx`.

**Instrumentation beyond completion/snooze-count**, added after those three gaps were named explicitly: `Reminder.snoozeHistory` (`{ at, minutes }[]`, capped at `MAX_SNOOZE_HISTORY_ENTRIES`) records how far EACH postponement actually pushed the task, not just that one happened — `snoozeReminder()` computes `minutes` from the moment of snoozing, which handles both the fixed-minutes and "tomorrow" presets uniformly. `Reminder.notifiedAt` and `Reminder.openedAt` distinguish "a notification reached the device" (stamped by `NotificationResponseHandler`'s received listener, keyed on the `reminderId` every scheduled notification's payload already carries) from "the user looked at it" (stamped by `reminder-detail.tsx` on mount, via `RemindersContext`'s `markOpened`) — two different facts a plain `completed`/`snoozeCount` pair conflates. Real, accepted limit on `notifiedAt`: `addNotificationReceivedListener` only fires while the app process is alive, so a notification delivered to a fully killed app's tray goes unrecorded — this stamp is evidence a notification fired, not proof it's the only time it did.

**Write serialization**: `services/ReminderService.ts` has a private `withWriteLock()` queue guarding `rescheduleAllFutureReminders()`, `markNotifiedById()`, `markOpenedById()`, and `markDoneById()` — added after a real race surfaced building the instrumentation above (`markDoneById()` was added later as hardening — see `system_learnings.md`, 2026-09-23 entry; it was not the cause of that entry's bug). All four do an independent "load the full array, mutate one reminder, save the full array" cycle, and nothing serialized them: a killed app cold-started straight into `reminder-detail` via a notification tap runs the provider's mount-time reschedule sweep and the screen's own open-stamp at the same moment, and whichever finished saving *last* used to win, silently discarding the other's write. **Not yet applied to every writer in the file** (`addReminder`, `editReminder`, `deleteReminder(s)`, `toggleComplete`, `snoozeReminder`, `updateSnoozeById` all share the same class of risk in theory) — only the four above are known to run concurrently with each other. Widening the queue to cover every writer is unstarted, not assumed done. Note the lock only serializes writes *within one JS runtime* — it does not, and cannot, close the separate cross-process race between the headless `TaskManager` task and the foreground listener (see `system_learnings.md`'s 2026-08-23 "Whole-array read-modify-write races" entry); that gap is mitigated, not eliminated, by `RemindersContext`'s AppState-`active` reload.

**Telemetry is two vendors, one switch, and no user content.** PostHog (product analytics) and Sentry (crashes) sit behind `services/AnalyticsService.ts` and `services/CrashReportingService.ts`; no screen imports either SDK. Both no-op entirely without their `EXPO_PUBLIC_` credentials, which is the state under Jest and in local dev — the suite constructs no client and makes no network call. `services/telemetryConsent.ts` holds the single opt-out (default ON, switch in Settings → Privacy) in its own module purely to break the import cycle that would exist if it lived in `ReminderService` alongside the other settings, since `ReminderService` itself emits events. The rule that keeps reminder content out is a type, not a convention: `AnalyticsProps` accepts only primitives, so passing a whole reminder fails to compile, and `utils/analyticsProps.ts` is the only thing that builds reminder-shaped properties. Crash reports get a second gate — `scrubEvent()` drops the user object and request bodies and redacts any string holding Malayalam script (in this app always user content) or a phone-number shape, because the realistic leak is an exception message that interpolated a title, not a deliberate capture. Event names live in one closed catalogue (`constants/analytics.ts`), and `constants/analytics.test.ts` fails the build if any declared event loses its last emitter — a catalogued event with no call site renders a chart that reads zero forever and gets believed. Two call-site rules worth knowing before adding one: `nl_parse_result` fires at save rather than at parse, because the parser runs on every keystroke; and `setting_changed` is one event with a `setting`/`value` pair rather than one event per switch, with free-text settings (the user's name) and the quiet-hours window itself deliberately never sent. Rationale, the full metric list and the cost comparison against Firebase/Crashlytics: `docs/analytics-and-crash-metrics.md`. Unproven on hardware — D79/D80/D81 in `device-tests/data-safety.md`.

**Mobile data layer**: `RemindersContext` (`contexts/RemindersContext.tsx`) wraps `ReminderService` (`services/ReminderService.ts`), which is the single source of truth for AsyncStorage reads/writes, all persisted settings, and `expo-notifications` scheduling/permissions/channels. All reminder CRUD and settings access goes through the context — screens never call the service directly. One exception writes behind the context's back: the tray actions in `NotificationResponseHandler` call `markDoneById`/`updateSnoozeById` directly, so they must call `refreshFromStorage()` afterwards, or the list stays stale and the next in-app save overwrites the change (see `system_learnings.md`, 2026-09-23). `RemindersProvider` must wrap `SharedTextProvider` in the provider tree (see Testing below) since `SharedTextContext` reads settings via `useReminders()`.
...
**System-wide text selection ("Remind Me" menu)**: Android's `ACTION_PROCESS_TEXT` puts the app in the floating text-selection toolbar of every other app. Two parts: `artifacts/mobile/plugins/withProcessText.ts` (config plugin — adds the intent-filter and the toolbar label to MainActivity) and `artifacts/mobile/modules/process-text/` (a **local Expo module**, Android only — the selected text arrives as an Intent *extra*, which React Native's Linking API cannot read, so native code is required). `SharedTextContext` publishes the text on the same `sharedText` channel as the share sheet, so `QuickAddInput` fills in either way. The module consumes the intent after reading it, so a rotation does not re-insert handled text. This needs a native rebuild — a Metro reload cannot add an intent-filter.
...
**Key utils (`utils/`):** `parseNaturalLanguage.ts`, `malayalamDateParser.ts`, `getFontFamily.ts` — see Architecture decisions above. Also `adherenceStats.ts`/`adherenceCopy.ts` — see "Adherence is derived, not logged" below. Also `formatDatetime.ts` (formats an ISO datetime as "Today · HH:MM" / "Tomorrow · HH:MM" / "Mon D · HH:MM").
...
- **Google Drive backup (B3)** lives entirely in `services/DriveBackupService.ts` (spec `docs/superpowers/specs/2026-09-25-google-drive-backup-design.md`, OAuth setup `docs/setup/google-drive-oauth.md`). Sign-in only works on a build whose signing SHA-1 is registered as an Android OAuth client — both the EAS keystore (`88:84:…:EC:59`) and the local debug keystore (`5E:8F:…:F6:25`, used by `expo run:android` debug *and* release variants) are registered as of 2026-09-25. A build signed with any other key fails sign-in with `DEVELOPER_ERROR` (code 10). On this repo's test phone `adb shell pm clear` is blocked (MIUI); wipe app data for a fresh-install test with `adb shell run-as com.curios.remindme rm -rf databases shared_prefs files no_backup` after a force-stop. The library's Expo config plugin is deliberately **not** in `app.json`: without options it applies Firebase config, and with options it requires an iOS `iosUrlScheme` that doesn't exist until an iOS OAuth client is created; Android needs nothing from it. A new route also needs `.expo/types/router.d.ts` regenerated before `pnpm run typecheck` accepts it locally (gitignored; `npx expo start` regenerates it).
