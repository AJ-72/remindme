...
- Android requires explicit notification channel setup; see `ensureNotificationChannels()` in `ReminderService.ts` — there's a legacy channel migration to handle.
- **The notification sound is the user's to choose, and Android's to own.** The app still ships `alarm.wav` as the channel default, but Settings → **Notification sound** (`app/(tabs)/settings.tsx`) calls `openNotificationChannelSettings()` (`ReminderService.ts`), which sends `android.settings.CHANNEL_NOTIFICATION_SETTINGS` via `Linking.sendIntent` for the channel `channelIdForAlarm()` picks, so the system's own sound picker offers every sound on the phone. The app cannot list or set those sounds itself: channel config is cached immutably by channel ID for the life of the install, so a runtime sound change is silently ignored — this is why the feature is a deep link rather than an in-app list. Android-only (iOS has no per-channel screen); the row is disabled while the alarm setting is off, since the silent/vibrate-only channels carry no sound. Some OEM ROMs refuse the channel intent, so there is a fallback to `Linking.openSettings()`.
